...
| HN-AUTH-011 | Wave 1 | A. Authentication & Account | Refresh token | Implemented & Working | 🟢 COMPLETED | P2 | V1.0 | Remaining: Android/client E2E when env available | PARTIALLY VERIFIED | AUTH011 host tests | Rotate-on-use: refresh JWT embeds unique `jti`; Redis (shared `_get_redis`) + in-process fallback atomically consumes jti before issuing successor; replay rejected; multi-device sessions preserved (no `token_version` bump on refresh); logout/`tv` still global revoke; AUTH011-BE host PASS; Flutter client already replaces stored refresh on success — unchanged; Android AUTH011-AND PENDING |
...
| HN-AUTH-017 | Wave 1 | A. Authentication & Account | Standalone verify-otp endpoint | Implemented & Working | 🟢 COMPLETED | P3 | V1.0 | Remaining: Android/provider E2E when env available | PARTIALLY VERIFIED | AUTH17 host tests | `POST /auth/verify-otp` consumes OTP for purposes `auth`/`register`, no tokens; `AuthApi.verifyOtp` client; login/register screens keep session issuance; AUTH17-BE + FL host PASS; Android AUTH17-AND PENDING |
...
| HN-AI-009 | Wave 3 | H. AI Health Assistant | AI rate limiting | Implemented & Working | 🟢 COMPLETED | P2 | V1.2 | Remaining: Android/client E2E when env available; product may tune AI_QUOTA_* env | PARTIALLY VERIFIED | AI009 host tests | Fail-closed per-user AI product quota (Redis Lua admit; shared `_get_redis`); default impl `AI_QUOTA_LIMIT=40` / `AI_QUOTA_WINDOW_SECONDS=86400` (env-overridable — not regulatory policy); covers provider-consuming `/ai/chat` + `/ai/health-guidance`; `/ai/report` excluded (no provider call); auth principal owns quota (ignores client user/family ids); consume-before-provider; AI009-BE host PASS; Flutter unchanged; Android PENDING |
...
| HN-SET-003 | Wave 1 | V. Settings | Privacy settings | Implemented & Working | 🟢 COMPLETED | P2 | V1.0 | Remaining: Android E2E / TalkBack | PARTIALLY VERIFIED | HN-SET-003 | Privacy hub `/home/settings/privacy`: consent status (read-only), legal docs, data export, delete account, OS notification settings deep-link; no fake analytics/AI toggles; SET03 host PASS; Android PENDING |
| HN-SET-004 | Wave 1 | V. Settings | Security settings | Implemented & Working | 🟢 COMPLETED / PARTIALLY VERIFIED | P3 | V1.0 | Remaining: Android device biometric E2E (enrollment + real auth) | PARTIALLY VERIFIED | HN-SET-004 | Optional biometric app-lock (device-local Hive bool); enable/disable require platform auth; `/home` gated via `/auth/app-lock`; no PIN (not in Remaining AC); SET004-FL host PASS; Android biometric E2E BLOCKED (emulator fingerprint HAL present, 0 enrolled prints) |
...
| HN-UX-002 | Wave 1 | W. Onboarding / UX | Welcome / onboarding | Implemented & Working | 🟢 COMPLETED | P2 | V1.0 | Remaining: Android E2E on stable emulator/device | PARTIALLY VERIFIED | HN-UX-002 | Optional guided 5-step tour after consent (`/auth/onboarding`); Hive `onboarding_completed`; Skip/Finish; a11y labels; UX02-FL host PASS; consent remains authoritative; Android E2E PENDING |
...
| HN-UX-006 | Wave 1 | W. Onboarding / UX | Accessibility | Implemented & Working | 🟢 COMPLETED | P2 | V1.0 | Remaining: TalkBack device E2E / full contrast QA | PARTIALLY VERIFIED | HN-UX-006 | Icon-only labels (AI send, drawer edit, records FAB, Rx close, med clear, eRx delete/revoke); emergency Call ≥48dp + named semantics; home chips ≥40dp; status not color-only; metric cards text-scale friendly; UX06-FL host PASS; TalkBack PENDING |
...
| HN-INFRA-006 | Wave 1 | X. Technical Infrastructure | Docker Compose | Implemented & Working | 🟢 COMPLETED | P1 | V1.0 | Remaining: Runtime compose up when Docker available; prod k8s/VM out of scope | PARTIALLY VERIFIED | LOCAL_DEVELOPMENT.md | Local Compose hardened: network, pg/redis/backend health (`/ready`), service DNS, env examples, alembic.ini secret removed; static INFRA06 validator PASS; Docker CLI absent in CI host → runtime PENDING |
...
| HN-INFRA-009 | Wave 5 | X. Technical Infrastructure | CI/CD | Implemented & Working | 🟢 COMPLETED / PARTIALLY VERIFIED | P2 | V2.0 | Remaining: Observe green run on GitHub-hosted Actions | PARTIALLY VERIFIED | HN-INFRA-009 | `.github/workflows/ci.yml` on push/PR: backend pytest (Py3.12 + requirements-dev) + Flutter `pub get`/`test`/`analyze` (stable); placeholder CI env only (no prod secrets/deploy); local pytest + flutter suites PASS; GitHub-hosted execution NOT YET EXECUTED |
